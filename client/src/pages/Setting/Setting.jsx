...
function Setting(props) {
    ...
    const infoFormik = useFormik({
        ...
        onSubmit: async (values) => {
            const { data } = await api.updateUserApi(currentUser.uuid, 'information', values);

            setMessageFrDb(data.message);
            dispatch({ type: 'SIGNIN', payload: { result: { ...data.newUser }, token: JSON.parse(localStorage.getItem('profile'))?.token } });
        },
    });
    const passwordFormik = useFormik({
        ...
        onSubmit: async (values) => {
            const { data } = await api.updateUserApi(currentUser.uuid, 'password', values);
            setMessageFrDb(data.message);
        },
    });
    return (
        <div className="setting-page">
            <div className="setting-page--paper pd-left-3 pd-right-3 shadow">
                <ul className="setting-page--paper--options">
                    ...
                    }} className={`${currentTab == 'information' && 'chosen shadow'} `}><h2>Thông tin cá nhân</h2></li>
                    <li onClick={() => {
                    ...
                    }} className={`${currentTab == 'password' && 'chosen shadow'} `}><h2>Mật khẩu</h2></li>
                </ul>



                <div className="setting-page--paper--forms">
                    <form onSubmit={currentTab === 'information' ? infoFormik.handleSubmit : passwordFormik.handleSubmit}>
                        {messageFrDb.message && (
                        ...
                        {currentTab === 'information' ? (
                            <>
                                <label htmlFor="imageUrl" className="setting-page--paper--forms__image pointer">
                                    <img referrerPolicy='no-referrer' src={infoFormik.values.imageUrl} width='200' height='200' alt="" className="" />
                                    <input
                                        type="file"
                                        name="imageUrl"
                                        className='dp-none'
                                        ...
                                        onChange={(e) => {
                                            convertImageToBase64(e.target.files[0], result => {
                                                result && infoFormik.setValues({ ...infoFormik.values, imageUrl: result })
                                            });
                                        }}
                                    />
                                </label>
                                <div className={`${width < 768 && "flex-column"} signin-form--control`}>
                                    ...
                                    {infoFormik.touched.name && infoFormik.errors.name ? (
                                        <div className="signin-form--control__error">{infoFormik.errors.name}</div>
                                    ) : null}
                                </div>
                                <div className={`${width < 768 && "flex-column"} signin-form--control`}>
                                    ...
                                    {infoFormik.touched.phone && infoFormik.errors.phone ? (
                                        <div className="signin-form--control__error">{infoFormik.errors.phone}</div>
                                    ) : null}
                                </div>
                                ...
                                <div className={`${width < 768 && "flex-column"} signin-form--control`}>
                                    ...
                                    >
                                        <div>
                                            <label className="radio-formik">
                                                <input type="radio" name="gender" value="Nam" defaultChecked={infoFormik.values.gender === "Nam" ? true : false} /> Nam
                                        </label>
                                            <label className="radio-formik">
                                                <input type="radio" name="gender" value="Nữ" defaultChecked={infoFormik.values.gender === "Nữ" ? true : false} /> Nữ
                                        </label>
                                            <label className="radio-formik">
                                                <input type="radio" name="gender" value="Khác" defaultChecked={infoFormik.values.gender === "Khác" ? true : false} /> Khác
                                        </label>
                                        </div>
                                    </div>
                                    ...
                                    {infoFormik.touched.gender && infoFormik.errors.gender ? (
                                        <div className="signin-form--control__error">{infoFormik.errors.gender}</div>
                                    ) : null}
                                    ...
                                    <div className={`${width < 768 && "flex-column"} signin-form--control`}>
                                        ...
                                        />
                                        {
                                            passwordFormik.touched.old_password && passwordFormik.errors.old_password ? (
                                                <div className="signin-form--control__error">{passwordFormik.errors.old_password}</div>
                                            ) : null
                                        }
                                    </div>
                                    <div className={`${width < 768 && "flex-column"} signin-form--control`}>
                                        ...
                                        />
                                        {
                                            passwordFormik.touched.password && passwordFormik.errors.password ? (
                                                <div className="signin-form--control__error">{passwordFormik.errors.password}</div>
                                            ) : null
                                        }
                                    </div>
                                    <div className={`${width < 768 && "flex-column"} signin-form--control`}>
                                        ...
                                        {passwordFormik.touched.confirm_password && passwordFormik.errors.confirm_password ? (
                                            <div className="signin-form--control__error">{passwordFormik.errors.confirm_password}</div>
                                        ) : null}
                                    </div>
                                </>
                            )}
                        <button type="submit" className={`button-submit ${width <= 1024 && 'full-width'}`}>Xác nhận</button>
                    </form>
                </div>
            </div>
        </div>
