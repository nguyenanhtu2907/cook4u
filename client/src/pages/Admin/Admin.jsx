...
function Admin(props) {
    ...
    const removeReport = async (uuid) => {
        ...
        setReports([...reports.filter((report) => report.uuid !== data.data.uuid)]);
    };
                ...
                <Table className="admin-page--paper--table ">
                    <TableHead>
                        <TableRow>
                            ...
                            {width > 767 && <TableCell align="center">Thời gian</TableCell>}
                            ...
                            return (
                                <TableRow key={report.uuid}>
                                    <TableCell component="th" scope="row">
                                        <Link
                                            to={`/user/${report.userReport.uuid}`}
                                            className="big-card--content__author "
                                        >
                                            <div className="big-card--content__author__image">
                                                <img
                                                    ...
                                                    width="50"
                                                    height="50"
                                                    src={report.userReport.imageUrl}
                                                    alt=""
                                                />
                                            </div>
                                    ...
                                    {width > 767 && (
                                        <TableCell align="center">{`${report.createdAt.getDay()}/${
                                            report.createdAt.getMonth() + 1
                                        }/${report.createdAt.getFullYear()}`}</TableCell>
                                    )}
                                    <TableCell align="center">
                                        <Button>
                                            <Link to={`/post/${report.target}`}>Link</Link>
                                        </Button>
                                    </TableCell>
                                    <TableCell align="center">
                                        <IconButton onClick={() => removeReport(report.uuid)}>
                                            <DeleteForeverIcon />
                                        </IconButton>
                                    </TableCell>
                                </TableRow>
